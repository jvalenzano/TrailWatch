...
```yaml
required_context:
  - path: docs/UI/trailwatch-component-architecture.md
  ...
  - path: docs/UI/trailwatch-frontend-track.md
...
optional_context:
  ...
  - path: docs/UI/TrailWatch_UI_Strategy.md
...
- maplibre-gl
- pmtiles (for offline tile support)
...
entry:
  - Phase 1 complete
  - MapLibre + Protomaps tiles configured
```
...
- src/components/map/MapView.tsx (MapLibre GL JS wrapper)
- src/components/map/ReportMarker.tsx (marker layer)
